...
namespace ObsStrawket.Test.Specs {
  public class StopOutputTest {
    ...
    public async Task TestAsync() {
      await SpecTester.TestAsync(new StopOutputFlow());
    }
  }

  internal class StopOutputFlow : ITestFlow {
    public async Task RequestAsync(ObsClientSocket client) {
      await Task.Delay(100).ConfigureAwait(false);
      _ = await client.StopOutputAsync(outputName: GetOutputListFlow.OutputName).ConfigureAwait(false);
